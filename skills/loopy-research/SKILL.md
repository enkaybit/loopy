...
This skill resolves unknowns where the answer **exists somewhere and needs to be found** — prior art, external constraints, codebase patterns, competitive landscape. For unknowns that need to be **built and experienced** to validate (UI/UX feel, interaction design, behavioral validation), use `loopy-spike` instead.

## When to Use

- After `loopy-requirements` produces a PRD with open questions that can be answered through research
...
1. **Categorize before investigating** — Not all questions belong here. Technical implementation questions (how to query X, which API to use) belong in tech planning's codebase exploration. Questions that need to be experienced belong in `loopy-spike`. This skill handles scope, requirements, external research, and prior art questions.
...
| **Needs spiking** | Suggest `loopy-spike` | "How should the drag interaction feel?" / "Would users find this flow intuitive?" |
...
2. If invoked from requirements discovery workflow: return to requirements discovery's Phase 5 transition (requirements discovery presents the next set of options).
...
- **All questions need spiking:** Nothing to research. Suggest invoking `loopy-spike` instead and skip Phase 3.
...
When invoked from requirements discovery: return to requirements discovery's transition (requirements discovery presents its own options).
...
| Researching questions that need to be experienced | Suggest `loopy-spike` — research can't answer "how should this feel?" |
